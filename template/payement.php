<div class='checkout'>
  <div class='order'>
    <h2>Paiement</h2>
    <h5>Commande n°010101</h5>
    <ul class='order-list'>
      <?php foreach($_SESSION['basket'] as $key => $value) : ?>
        <?php foreach($value as $keys => $values) : ?>
          <li><h4><?= $values['title'] ?></h4><h5><?= $values['price'] ?> €</h5></li>
        <?php endforeach ?>
      <?php endforeach ?>
    </ul>
    <h5 class='total'>Total</h5><h1><?= $totalPrice ?> €</h1>
  ...
  <div id='payment' class='payment'>
    ...
    <div class='card-form'>
        ...
        <input type='text' id='cardnumber' name='cardnumber' placeholder='1234 5678 9123 4567' pattern='\d*' title='Numero de carte' />
      </p>
        <p class='field space'>
      ...
      <button class='button-cta' title='Confirm your purchase'><span>ACHETER</span></button>
    </div>
  </div>
  <div id='paid' class='paid'>
    ...
    <h2>Votre paiement à bien été pris en compte !</h2>
    <h2>MERCI !</h2>
  </div>
</div>
